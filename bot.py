...
def main():
    ...
    menu_pattern = r"^(👤 Tài Khoản|🎯 Làm Nhiệm Vụ|🎁 Đổi Quà|📜 Lịch Sử|🏆 BXH|📖 Hướng Dẫn|🕺 Hỗ Trợ / Liên Hệ)$"
    app.add_handler(MessageHandler(filters.Regex(menu_pattern), native_menu_handler))
    admin_menu_pattern = r"^(👥 Người Dùng|🎯 Nhiệm Vụ|💰 Quản Lý Xu|🎁 Quà Tặng|🔑 Key / File|📊 Thống Kê|📢 Thông Báo|⚙️ Cài Đặt|🛡 Bảo Mật|🔗 Api Rút Gọn|📜 Lịch Sử Hệ Thống|📦 Đơn Đổi Quà|🏠 Menu Người Dùng)$"
    app.add_handler(MessageHandler(filters.Regex(admin_menu_pattern), admin_native_menu_handler))
